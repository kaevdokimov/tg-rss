...
# Определяем расписание запусков (по умолчанию 12:00 и 19:00)
# Формат: "HH:MM,HH:MM" или через переменную ANALYZER_SCHEDULE_TIMES
SCHEDULE_TIMES="${ANALYZER_SCHEDULE_TIMES:-12:00,19:00}"
...
if [ ${#SCHEDULE_HOURS[@]} -eq 0 ]; then
    log "ОШИБКА: Не удалось распарсить расписание. Используем по умолчанию: 12:00, 19:00"
    SCHEDULE_HOURS=(12 19)
