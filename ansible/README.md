...
1. Выберите окружение (dev/staging/prod) и отредактируйте соответствующий файл:
```bash
# Для разработки
vim ansible/inventory/dev/hosts.ini

# Для продакшена
vim ansible/inventory/prod/hosts.ini
```

Пример конфигурации:
```ini
[tg_rss_servers]
your-server ansible_host=your-server-ip ansible_user=deploy ansible_port=22
```
...
# Инициализация vault (если нужно)
make vault-init

# Настройка качества кода (рекомендуется)
make pre-commit-install

# Развертывание в dev окружение (по умолчанию)
make deploy

# Или явно указать окружение
make dev-deploy     # Development
make staging-deploy # Staging
ENVIRONMENT=prod make deploy  # Production

# Для CI/CD используется ENVIRONMENT=prod автоматически
```

## 🌍 Окружения

Проект поддерживает три окружения с разными настройками:

### Development (разработка)
- Минимальные системные требования
- Детальное логирование (`DEBUG`)
- Релаксированные настройки безопасности
- `make dev-deploy` или `ENVIRONMENT=dev make deploy`

### Staging (предпрод)
- Балансированные настройки производительности
- Полный мониторинг и алертинг
- Средний уровень безопасности
- `make staging-deploy` или `ENVIRONMENT=staging make deploy`

### Production (продакшн)
- Максимальная производительность и безопасность
- Полный мониторинг и алертинг
- Строгие правила безопасности
- `make prod-deploy` или `ENVIRONMENT=prod make deploy`
- **Используется в CI/CD по умолчанию**
