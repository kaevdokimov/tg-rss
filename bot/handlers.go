...
func StartCommandHandler(bot *tgbotapi.BotAPI, dbConn *sql.DB, interval int) {
	...
	for update := range updates {
		...
		if update.CallbackQuery != nil {
			...
			handlerLogger.Debug("Callback received",
		...
		handlerLogger.Debug("Message received",
...
func handleStart(bot *tgbotapi.BotAPI, dbConn *sql.DB, username string, chatId int64) {
	...
	handlerLogger.Info("Пользователь подключился к боту",
...
func handleAddSource(bot *tgbotapi.BotAPI, dbConn *sql.DB, chatId int64, link string) {
	...
	if err != nil {
		handlerLogger.Error("Ошибка при добавлении источника", "error", err)
...
func handleSubscribeAll(bot *tgbotapi.BotAPI, dbConn *sql.DB, chatId int64) {
	...
	for _, source := range sources {
		...
		isSubscribed, err := db.IsUserSubscribed(dbConn, chatId, source.Id)
		if err != nil {
			handlerLogger.Error("Ошибка при проверке подписки на источник",
				"source_id", source.Id,
				"error", err)
			errorsCount++
			continue
		...
		err = db.SaveSubscription(dbConn, subscription)
		if err != nil {
			handlerLogger.Error("Ошибка при добавлении подписки на источник",
				"source_id", source.Id,
				"error", err)
			errorsCount++
			continue
...
• "📋 Мои источники" - просмотр и подписка на источники
• "📝 Мои подписки" - управление вашими подписками
• "🚀 Быстрый старт" - подписка на популярные источники
...
• Источник является ссылкой на новость
• Время в формате: "N мин", "N ч", "N дн"

📬 *Доставка новостей:*
Новые новости приходят автоматически списком каждые 15 минут. Все новости, накопленные за этот период, отправляются одним или несколькими сообщениями (если список новостей превышает лимит Telegram в 4096 символов). Нумерация новостей продолжается между сообщениями для удобства чтения.

💡 *Примечание:* После подписки на источники новости начнут приходить автоматически. Первая отправка может занять до 15 минут.`
