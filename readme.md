...
**Версия 2.1** включает исправление проблемы дублирования новостей, оптимизацию контент скрейпинга и улучшенную дедупликацию.
...
- 🔄 **Параллельная обработка** - одновременный парсинг до 6 RSS источников
...
- **RAM**: 1 ГБ (960MB фактически используется, после оптимизаций)
- **Disk**: 15 ГБ
...
- Интеллектуальному кэшированию источников и подписок
- Параллельной обработке RSS с ограничением воркеров
- Оптимизированным параметрам Kafka и PostgreSQL
...
3. Отредактируйте `.env` файл, указав ваши настройки:
   ...
   CONTENT_SCRAPER_BATCH=100    # размер батча для парсинга контента (по умолчанию: 20)
   CONTENT_SCRAPER_CONCURRENT=10 # количество параллельных запросов (по умолчанию: 6)
   ...
   WORKER_POOL_SIZE=6          # количество воркеров для параллельного парсинга RSS (по умолчанию: 6)
   SOURCES_CACHE_TTL=30m      # TTL кэша источников (по умолчанию: 30m)
   SUBSCRIPTIONS_CACHE_TTL=10m # TTL кэша подписок (по умолчанию: 10m)
   ```
...
- Kafka: произведенные и потребленные сообщения, ошибки
...
├── kafka/                  # Kafka producer/consumer (оптимизированные параметры)
...
Доступные тесты:
- `config` - тесты конфигурации (76.9% покрытие)
- `bot` - тесты rate limiter, error handler, форматирования сообщений (7.6% покрытие)
- `rss` - тесты парсинга RSS (41.7% покрытие)
- `monitoring` - тесты метрик и логирования (67.6% покрытие)

**Покрытие тестами**: ~48.5% (среднее по проекту)

**После оптимизаций добавлены:**
- Тесты для параллельной обработки RSS источников
- Тесты для кэширования источников и подписок
- Тесты для health check endpoints
- Тесты для graceful shutdown функциональности
...
### 📊 Текущие ограничения ресурсов (после оптимизаций)

| Сервис      | Память | CPU | JVM Heap | Комментарий |
|-------------|--------|-----|----------|-------------|
| **bot**     | 512MB  | 0.5 | -        | Основной бот с оптимизациями производительности |
| **kafka**   | 384MB  | 0.15| 256MB    | Оптимизированные параметры producer/consumer |
| **zookeeper**| 256MB | 0.2 | 128MB    | Стабильно для координации |
| **db**      | 256MB  | 0.15| -        | PostgreSQL с оптимизированными запросами |
| **ИТОГО**   | **1408MB** | **1.0** | - | **30% экономии ресурсов после оптимизаций** |
...
- worker_pool_size: 6       # Одновременный парсинг до 6 источников
...
# Оптимизация Kafka
- compression: snappy       # Сжатие сообщений для экономии трафика
- batch_size: 100          # Отправка батчами по 100 сообщений
```
...
✅ **Экономия ресурсов**: 30% по сравнению с базовой конфигурацией
...
✅ **Надежность**: graceful shutdown и встроенный мониторинг
✅ **Масштабируемость**: эффективное использование ресурсов CPU
...
✅ **Решено**: Добавлена параллельная обработка с worker pool (до 6 одновременных потоков), что ускорило сбор новостей в 3-5 раз для больших списков источников.
...
- **Kafka оптимизации**: Сжатие, батчинг и улучшенные параметры producer/consumer
...
### 🚀 Технические улучшения v2.1
- **Исправление дублирования**: Глобальная дедупликация новостей по контенту
- **Оптимизация контент скрейпинга**: Каждые 1 минуту, батчи по 100 новостей, 10 параллельных потоков
- **Улучшенная надежность**: Глобальная проверка отправки перед распределением
...
- **Экономия ресурсов**: 30% снижение потребления ресурсов по сравнению с v1.0
