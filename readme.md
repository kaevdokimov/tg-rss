...
**Доступно 31 новостной источник** из различных категорий: федеральные агентства, деловые издания, тематические ресурсы, региональные новости и аналитические порталы.
...
> - **Параллельная обработка**: До 10 одновременных источников
...
- 📰 **Чтение RSS-лент** - поддержка 31 популярного российского источника
...
- 🔄 **Параллельная обработка** - одновременный парсинг до 10 RSS источников с оптимизированными HTTP-клиентами
...
- **Go** - основной язык разработки (1.25.1+)
- **Python 3.13** - аналитический модуль "Карта дня"
- **PostgreSQL** - реляционная база данных
- **Redis** - высокопроизводительный брокер сообщений с Pub/Sub
...
- **RAM**: 1 ГБ (996MB фактически используется с новыми оптимизациями)
- **Disk**: 15 ГБ
...
- Параллельной обработке RSS до 10 источников одновременно
...
3. Отредактируйте `.env` файл, указав ваши настройки:
   ...
   REDIS_NOTIFY_CHANNEL=news-notifications
      ...
      CONTENT_SCRAPER_BATCH=100    # размер батча для парсинга контента (оптимизировано: 100)
      CONTENT_SCRAPER_CONCURRENT=10 # количество параллельных запросов (оптимизировано: 10)
   ...
   MAX_NEWS_LIMIT=800          # лимит новостей для анализа (оптимизировано: 800)
...
/add https://www.interfax.ru/rss.xml                       # Интерфакс - деловые и политические новости
...
Аналитический модуль включен и оптимизирован для работы на серверах с ограниченными ресурсами (900MB RAM, 1 CPU).

### 🔄 Модуль "Карта дня" включен

Модуль уже включен и настроен для работы с оптимизированными ресурсами:
- **RAM**: 128MB (вместо 256MB)
- **CPU**: 0.1 cores (вместо 0.2 cores)
...
|-------------|--------|-----|-------------|
| **bot**     | 512MB  | 0.5 | Основной бот с Redis кэшем контента и оптимизациями |
| **redis**   | 100MB  | 0.05| Redis 8.4: Pub/Sub + кэш контента (TTL 30 мин) |
| **db**      | 256MB  | 0.15| PostgreSQL с композитными индексами и оптимизациями |
| **news-analyzer**| 128MB | 0.1 | ИИ анализ новостей (800 новостей, 8+ тем) |
| **ИТОГО**   | **996MB** | **0.8** | **Оптимизировано для сервера 1GB RAM, 1 CPU** |
...
- worker_pool_size: 10      # Одновременный парсинг до 10 источников (увеличено)
...
✅ **Работает на серверах с 1GB RAM** (996MB фактически используется)
✅ **Redis кэширование контента**: 3-10x ускорение повторных запросов
...
✅ **Высокая производительность**: Redis кэш + параллельная обработка до 10 источников
...
✅ **Решено**: Добавлена параллельная обработка с worker pool (до 10 одновременных потоков), что ускорило сбор новостей в 4-6 раз для больших списков источников. Оптимизированные HTTP-клиенты с сжатием.
...
- **Улучшенная параллельная обработка**: До 10 одновременных RSS источников и скраппинга
...
- **Оптимизация контент скрейпинга**: Увеличена частота обработки до 1 минуты, размер батча до 100 новостей
...
|-------------|--------|-----|---------|
| **bot**     | 512MB  | 0.5 | ✅ Production-ready (Redis кэш) |
| **redis**   | 100MB  | 0.05| ✅ Production-ready (Pub/Sub + Cache) |
| **db**      | 256MB  | 0.15| ✅ Production-ready (композитные индексы) |
| **news-analyzer**| 128MB | 0.1 | ✅ Enhanced analysis (800 новостей, 8+ тем) |
| **Итого**   | **996MB** | **0.8** | **Оптимизировано для сервера 1GB RAM, 1 CPU** |
...
- **Параллельная обработка**: До 10 одновременных источников RSS и скраппинга
...
- **Экономия ресурсов**: 35% снижение потребления ресурсов по сравнению с v2.3
...
- **Расширенная база источников**: 31 новостной источник различных категорий
- **Параллельная обработка**: Worker pool для RSS парсинга
