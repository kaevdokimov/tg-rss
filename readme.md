...
docker exec -it kafka kafka-console-consumer --bootstrap-server localhost:29092 --topic news-notifications --from-beginning
```
...
### Компоненты:
- **RSS Poller** - парсит RSS источники и отправляет новости в Kafka (топик news-items)
